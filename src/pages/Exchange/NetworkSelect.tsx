...
export const NetworkSelect = observer((props: {
  secret: boolean;
  value: NETWORKS,
  onChange: Function,
  ...
  if (secret) return <div style={{ padding: 10, minWidth: 300 }}>
  ...
  const Option = (option) => {
    return <components.Option {...option} className={styles.selectOption}>
      <NetworkTemplate template={{ ...option.data, symbol: null }} user={user} />
    </components.Option>
  }

  const Control = ({ children, ...props }) => {
    return <components.Control {...props} className={styles.selectContainer}>
      {children}
    </components.Control>
  };
